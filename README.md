...
* поиск пути выполняется с помощью алгоритма A***:
    * `gScore[cell]` хранит стоимость пути от старта до клетки;
    * используется допустимая эвристика `simplification` — расстояние Чебышёва, подходящее для движения в 8 направлениях при одинаковой цене шага;
* для выбора следующей клетки используется очередь с приоритетом, где извлекается вершина с минимальным f;
* соседние клетки проверяются по 8 направлениям;
* посещённые клетки помечаются в closed, что предотвращает зацикливание;
* если путь найден, метод восстанавливает маршрут по массиву parent и возвращает его как List<Edge>;
...
Где:
* размер поля: всего V = W · H клеток;
* в худшем случае алгоритм может обработать большинство из этих клеток.

Для каждой обрабатываемой клетки выполняется просмотр до 8 соседей, и возможные операции вставки в очередь.
